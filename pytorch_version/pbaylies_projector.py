...
def project(
    ...
    *,
    ...
    for step in range(num_steps):
        ...
        # if np.round(dist.clone().detach().cpu(),2) == np.round(loss.clone().detach().cpu(),2):
        #     if np.var(list_dist[-10:]) < 0.0001:
        #         print(list_dist[-10:])
        #         print(np.var(list_dist[-10:]))
        #         print(dist,loss,"SAME")
        #         # return w_out
        #         break
...
def run_projection(
    ...
    if save_video:
        ...
        print (f'Saving optimization progress video "{outdir}/{os.path.basename(target_fname)[:-4]}_proj.mp4"')
